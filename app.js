...
const countries = [
  ...
  {code:"it",name:"Italy",continent:"Europe",food:"Pizza",foodName:"Pasta and pizza",foods:[{name:"Pasta",wiki:"Pasta"},{name:"Pizza",wiki:"Pizza"}],fact:"Italy did not qualify for the 2026 World Cup, even though it has won the tournament four times.",notQualified:true},
...
function countryDetail(code){
  ...
  const foodHtml=c.foods?`<div class="food-gallery">${c.foods.map(food=>`<figure><img class="food-img" data-wiki="${food.wiki}" src="assets/food-placeholder.svg" alt="${food.name}"><figcaption>${food.name}</figcaption></figure>`).join("")}</div>`:`<img class="food-img" ${c.foodImage?`src="${c.foodImage}"`:`data-wiki="${c.food}" src="assets/food-placeholder.svg"`} alt="${c.foodName}">`;
